    ...
    <>
      ...
      >
        <motion.img
          src={avatar}
          alt={user.name}
          className="w-28 h-28 rounded-full object-cover border-2 border-violet-500 mb-4"
        />

        <h2 className="text-2xl font-bold mb-4">@{user.username || user.name}</h2>
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.05 }}
          onClick={(e) => {
            e.stopPropagation(); // Prevent modal opening when clicking message button
            handleMessage();
          }}
          className="mt-auto px-6 py-2 bg-gradient-to-r from-violet-500 to-cyan-400 rounded-lg font-semibold text-white shadow-lg"
        >
          💬 Message
        </motion.button>
      </motion.div>
